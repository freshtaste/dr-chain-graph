...
def sample_L(L, adj_matrix, tau, rho, nu):
    ...
    # Compute neighbor summaries for each covariate
    L_nb = [adj_matrix @ L_old[:, d] for d in range(D)]

    # Update each L[:, d] separately
    for d in range(D):
        # Compute linear predictor for covariate d
        linpred = tau[d]
        for d2 in range(D):
            if d2 != d:
                linpred += rho[d, d2] * L_old[:, d2]
                linpred += nu[d, d2] * L_nb[d2]
        linpred += nu[d, d] * L_nb[d]
        # Convert to probabilities and sample
        prob = expit(linpred)
        L[:, d] = np.random.binomial(1, prob)
    return L
...
def sample_A(A, L, adj_matrix, gamma):
    ...
    for d in range(L.shape[1]):
        linpred += gamma[1 + 2 * d] * L[:, d] + gamma[2 + 2 * d] * L_nb[:, d]
    linpred += gamma[1 + 2 * L.shape[1]] * A_nb
...
def sample_Y1(Y, A, L, adj_matrix, beta, Atype='all'):
    ...
    if Atype == 'all' or Atype == 'gen':
        ...
        raise ValueError("Invalid Atype.")

    linpred = beta[0] + beta[1] * A_self + beta[2] * A_nb
    for d in range(L.shape[1]):
        linpred += beta[3 + 2 * d] * L[:, d] + beta[4 + 2 * d] * L_nb[:, d]
    linpred += beta[3 + 2 * L.shape[1]] * Y_nb
...
def sample_Y2(Y, A, L, adj_matrix, beta, Atype='all'):
    ...
    linpred = beta[0] + beta[1] * A_self + beta[2] * A_nb
    for d in range(L.shape[1]):
        linpred += beta[3 + 2 * d] * L[:, d] * A + beta[4 + 2 * d] * L_nb[:, d] * A_nb
    linpred += beta[3 + 2 * L.shape[1]] * A * A_nb
...
def sample_network_chain(
    ...
    D = 5  # Now using 5 covariates

    # Initialize chains
    L_chain = np.zeros((R + burnin_R, N, D), dtype=int)
    A_chain = np.zeros((R + burnin_R, N), dtype=int)
    Y_chain = np.zeros((R + burnin_R, N), dtype=int)

    # Initial values
    L = np.random.binomial(1, 0.5, size=(N, D))
