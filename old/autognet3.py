...
def fit_autog_models(Y, A, L, adj_matrix):
    L_models = []
    for k in range(3):
        X_Lk, y_Lk = prepare_features_L_k(L.copy(), k, adj_matrix.copy())
        model = LogisticRegression(penalty='none', solver='lbfgs', max_iter=10000)
        model.fit(X_Lk, y_Lk)
        L_models.append(model)
    X_Y = prepare_features_outcome_model(Y.copy(), A.copy(), L.copy(), adj_matrix.copy())
